...
async function promising(value) {
  ...
  if (typeof value !== "undefined") {
    const master = await fetch('https://api.github.com/repos/' + value +'/commits/master');
    const masterJSON = await master.json();
    const masterDATA = time(masterJSON);

    const release = await fetch('https://api.github.com/repos/' + value +'/releases');
    ...
    const topic = await fetch('https://api.github.com/repos/' + value +'/topics', {headers: {'Accept': 'Accept: application/vnd.github.mercy-preview+json'}});
    const topicJSON = await topic.json();
    const topicDATA = topics(topicJSON);

    arrayOriginal.push({"date": masterDATA, "download": releaseDATA, "other": repositoryDATA, "topic": topicDATA});
  }
...
const exploreData = async (filter) => {
  try {
    if ((typeof filter !== "undefined") && filter.length) {
      ...
      datum.sort((a: any, b: any) => (+new Date(a.date.old) - +new Date(b.date.old)));
